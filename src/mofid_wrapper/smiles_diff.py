...
Reports common classes of errors/differences in a calculated MOFid, more
specific than a flat 'err_smiles'. Ported from upstream mofid's
...
# See single_smiles_diff for how these errors are defined.
#
# The _extra suffix from find_closest_match indicates that a component has
# been replicated in the MOFid with a redundant copy containing the specified
# error.
DIFF_LEVELS = dict(
    {
        "same": 0,
...
def find_closest_match(smiles, preferred_list, extra_list):
    ...
    if best_match[2]:
        best_match[0] = best_match[0] + "_extra"
...
def multi_smiles_diff(smiles1, smiles2):
    # What are the differences between two dot-separated SMILES codes?
    if smiles1 == smiles2:
        return []
...
def single_smiles_diff(smiles1, smiles2):
    ...
    def radical_to_carb(smiles):
        return openbabel_replace(
            smiles, "[#6:1][#6D3:2](~[O-0:3])[O:4]", "[#6:1][#6:2](=[O:3])[O-:4]"
        )
    ...
    molec_type = "linker" if is_organic(smiles1) and is_organic(smiles2) else "node"
...
if __name__ == "__main__":
    ...
    if len(args) != 2:
        raise SyntaxError(
            "Run a diff between two SMILES strings. Be sure to quote to escape bash globbing, etc."
        )
