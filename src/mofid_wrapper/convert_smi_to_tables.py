...
Converts a database of MOFid strings into tables of the detected components,
keyed by name. Ported from upstream mofid's Python/convert_smi_to_tables.py.
"""
...
from mofid_wrapper.id_constructor import parse_mofid
    ...
    def parse(self, filename):
        with open(filename, "r") as f:
            for line in f:
                ...
                parsed["extra_topology"] = ",".join(parsed["topology"].split(",")[1:])
...
if __name__ == "__main__":
    ...
    if len(args) != 1:
        raise SyntaxError(
            "Extract info from a list of MOFid strings. Only a single .smi filename expected."
        )
